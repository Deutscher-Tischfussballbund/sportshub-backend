...
/**
 * Tier A (revised): a season belongs to a region, so its writes require admin of that region (or
 * global) — create gated on the target region via {@code @authz.canManageRegion(#seasonDto.federationId)},
 * update/delete on the season's region via {@code @authz.canManageSeason(#id)}. Reads stay open.
 *
...
class SeasonControllerSecurityTest {
    ...
    private static final String BODY = """
        {"name": "2024", "federationId": "region-x"}
        """;

    @Test
    void create_withoutToken_isUnauthorized() throws Exception {
        mockMvc.perform(post("/v1/seasons").contentType(MediaType.APPLICATION_JSON).content(BODY))
    ...
    void create_whenNotRegionManager_isForbidden() throws Exception {
        ...
        mockMvc.perform(post("/v1/seasons").with(jwt())
    ...
    @Test
    void update_whenNotSeasonManager_isForbidden() throws Exception {
        Mockito.when(authz.canManageSeason(any())).thenReturn(false);
        mockMvc.perform(put("/v1/seasons/season-x").with(jwt())
                .contentType(MediaType.APPLICATION_JSON).content(BODY))
            .andExpect(status().isForbidden());
    }
    ...
    void read_asAnyAuthenticatedUser_isAllowed() throws Exception {
        mockMvc.perform(get("/v1/seasons").with(jwt()))
