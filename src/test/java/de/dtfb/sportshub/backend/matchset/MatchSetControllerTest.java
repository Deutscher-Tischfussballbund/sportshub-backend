...
class MatchSetControllerTest extends de.dtfb.sportshub.backend.support.AuthorizedControllerTest {
    ...
    void setupEach() throws Exception {
        url = createMatchset().getResponse().getHeader("Location");
        assert url != null;
    }

    @Test
    void getAllMatchsets() throws Exception {
        mockMvc.perform(get("/v1/matchsets")).andExpect(status().isOk());
    }
    ...
    private MvcResult createMatchset() throws Exception {
        return mockMvc.perform(post("/v1/matchsets")
                .contentType(MediaType.APPLICATION_JSON)
                .content(String.format("""
                            {"setNumber": "1", "matchId": "%s", "homeScore": "5", "awayScore": "2"}
                    """, matchId)))
            .andExpect(status().isCreated()).andReturn();
    }
    //endregion
