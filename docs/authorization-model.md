...
1. **Authentication is shared, authorization is centralized.**
   ...
   never in Keycloak — because roles carry **scope** (region/club/competition ids), which is
...
| 🟩 **Function / activity** | "What *job* do you perform for this competition?" | functional roles: `competition_organizer` (+ future `referee`/`result_reporter`) |
...
COMPETITION   (Competition.id)  ── belongs to Season ── belongs to Federation (REGION)
```

- **`COMPETITION`** is the competition scope. **Leagues and tournaments are both `Competition`s**
  (`Competition → Season → Federation`); the league (match-days under `Round`) vs. tournament
  (pools/stages under the same Competition) distinction is structural *below* the Competition, so one
  `COMPETITION` scope covers both.
- An Competition resolves up to a REGION via `Competition.season.federation`, so a `region_admin`
  can manage competitions in their region and a global `admin` can manage all — this slots
  `COMPETITION` into the existing hierarchy for enforcement.
...
| `region_admin` | REGION | Manage a Landesverband: its clubs, teams, seasons, competitions. |
...
| Role | Why |
|---|---|
| `tournament_uploader` (CLUB) | Right idea (a functional "run results" role), wrong axis — anchored to CLUB instead of the competition. Superseded by `competition_organizer` (COMPETITION). |
| `region_tournament_uploader` (REGION) | Same; a `region_admin` already covers region-wide competition management. |
...
| `result_reporter` | TEAM / COMPETITION | if team-rep reporting needs to be distinct from `team_admin`. |
...
| Manage seasons, competitions, disciplines & locations in my region | Season/Competition/Discipline/Location | REGION | 🟦 region_admin |
...
### Tournament manager — organizers running an competition
| Action | Resource | Scope | Axis |
|---|---|---|---|
| Build/seed pools, rounds, stages | Pool/Round/Stage | COMPETITION | 🟩 competition_organizer |
| Schedule matches | Match | COMPETITION | 🟩 competition_organizer |
| Enter/correct results, publish standings | Match/MatchSet/Standing | COMPETITION | 🟩 competition_organizer |
...
| Action | Resource | Scope | Axis |
|---|---|---|---|
| Record set/point competitions, finalize score | MatchSet/MatchEvent/Match | MATCH | 🟩 referee (future) |
...
- **A — Config** ✅ **Done.** Split by scope along the federation tree: a **Category** is a global classification (admin-only); everything that hangs off a region — **Season, Competition, Discipline, Location** — is region-scoped (a region admin owns their slice). Each resolver lives in `@authz` and a region-less entity falls back to admin-only.
  ...
  - *Region config*:
    ...
    - **Competition** (`Competition → Season → Federation`): create → `canManageSeason(#competitionDto.seasonId)`, update/delete → `canManageCompetition(#id)`.
    - **Discipline** (`Discipline → Competition → … → Federation`; its `Category` is just a classification, not scope): create → `canManageCompetition(#disciplineDto.eventId)`, update/delete → `canManageDiscipline(#id)`.
    ...
    - Verified by `Season`/`Competition`/`Discipline`/`LocationControllerSecurityTest`. (Competition *competition data* — pools/matches/standings — is Tier C, gated on the owning Competition's scope incl. `competition_organizer`.)
...
- **C — Competition data** (Stage/Pool/Round/MatchDay/Match/MatchSet/MatchEvent) ✅ **Done.** Each entity resolves up its `@ManyToOne` spine to the owning Competition — `Stage → Discipline → Competition`, `Pool → Stage → …`, `Match → MatchDay → Round → …` — via `CompetitionResolver` (one component owns the 8 repos; the chain is eagerly fetched by a single load). The capability `@authz.canOrganize<Entity>` = the region/global admin above that competition **OR** an `competition_organizer` appointed to it. This is deliberately broader than `canManageCompetition` (competition meta — admins only): organizers run the competition, not the competition's place in the federation tree. Writes gated: create on the parent's resolver (e.g. Stage create → `canOrganizeDiscipline(#stageDto.disciplineId)`), update/delete on the entity's own (`canOrganizeStage(#id)`). `StandingController` is read-only (standings are computed) so it has no write gate — N/A like `ClubController`. MatchDay `create/update/delete` are Tier C; its `result`/`confirm` remain Tier D. Verified by `CompetitionAuthorizationIntegrationTest` (real cascade + `competition_organizer` allow, outsider/other-competition deny) and `CompetitionControllerSecurityTest` (all 21 write endpoints deny a non-organizer).
