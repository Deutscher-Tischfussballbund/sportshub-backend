...
Roles already exist: `ADMIN > REGION_ADMIN > CLUB_ADMIN > TEAM_ADMIN` (+ `LEAGUE_ADMIN`, renamed from `EVENT_ORGANIZER` then `COMPETITION_ORGANIZER` alongside the entity, later widened to a full "league admin" scoped to one league — see `docs/03-authorization-model.md` §4 and `docs/09-league-model.md` §4).
