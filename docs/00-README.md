...
| 11 | [Placement board](./11-placement-board.md) | decision | **Proposed**, not built. Visual/drag-and-drop rework of the region placements view: tier-stacked, side-by-side groups, an unplaced lane, per-tier randomize gated on actual Round/MatchDay existence (not the unenforced `Group.groupState` label). |
